...
header("Content-Disposition: attachment; filename=Consolidado INS-PEV-EVA-PCE-CER Total REG-CEN (Del $fInicial Al $fFinal)(Generado $fe H$h M$m).xls");
...
//                    echo '<hr/> Candidatos Inscritos en el Centro ' . $rowCen['CODIGO_CENTRO'][$c] . ': <hr/>' .
					$sqlIns = 'SELECT '
							. ' COUNT(INS.ID_CANDIDATO) AS TOTAL_INSCRITOS '
...
//                    echo '<hr/> Candidatos Evaluados en el Centro ' . $rowCen['CODIGO_CENTRO'][$c] . ': <hr/>' .
					$sqlEva = 'SELECT '
							. ' COUNT(EVA.ID_CANDIDATO) AS TOTAL_EVALUADOS '
...
//                    echo '<hr/> Candidatos Certificados en el Centro ' . $rowCen['CODIGO_CENTRO'][$c] . ': <hr/>' .
					$sqlCer = 'SELECT '
							. '	COUNT(NROIDENT) AS TOTAL_CERTIFICADAS '
							. 'FROM T_HISTORICO '
...
//                        echo '</pre>';
					...
					<tr>
						...
						<td <?php echo $classRegistros; ?>>
							<?php echo ($rowIns['TOTAL_INSCRITOS'][0] + $datosNCCERIns['TOTAL_INSCRITOS'][0]); ?>
						</td>
						<td <?php echo $classRegistros; ?>>
							<?php echo ($rowIns['TOTAL_INSCRITOS'][0] + $datosNCCERIns['TOTAL_INSCRITOS'][0]) - ($rowEva['TOTAL_EVALUADOS'][0] + $datosNCCERIns['TOTAL_EVALUADOS'][0]); ?>
						</td>
						<td <?php echo $classRegistros; ?>>
							<?php echo ($rowEva['TOTAL_EVALUADOS'][0] + $datosNCCEREva['TOTAL_EVALUADOS'][0]); ?>
						</td>
						<td <?php echo $classRegistros; ?>>
							<?php echo ($rowEva['TOTAL_EVALUADOS'][0] + $datosNCCEREva['TOTAL_EVALUADOS'][0]) - ($rowCer['TOTAL_CERTIFICADAS'][0] + $datosNCCEREva['TOTAL_CERTIFICADAS'][0]); ?>
						</td>
						<td <?php echo $classRegistros; ?>>
							<?php echo ($rowCer['TOTAL_CERTIFICADAS'][0] + $datosNCCERCer['TOTAL_CERTIFICADAS'][0]); ?>
						</td>
					</tr>
					<?php
				}
