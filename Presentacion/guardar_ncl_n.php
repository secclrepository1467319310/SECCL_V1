...
for ($i = 0; $i < $total; $i++) {
    ...
    $strSQL = "INSERT INTO DETALLES_POA (ID_NORMA,NIT_EMPRESA,ID_PLAN,FECHA_REGISTRO,ID_PROVISIONAL,USU_REGISTRO)
        VALUES ('$perinv[$i]','$nit_empresa','$cp','$fecha_registro','$provisional[0]','$id')";
...
for ($i = 0; $i < $total; $i++) {
    ...
    $strSQL = "INSERT INTO DETALLES_POA (ID_NORMA,NIT_EMPRESA,ID_PLAN,FECHA_REGISTRO,ID_PROVISIONAL,'USU_REGISTRO')
        VALUES ('$perinv[$i]','$nit_empresa','$cp','$fecha_registro','$provisional','$id')";
...
?>
<script type="text/javascript">
    window.location = "../Presentacion/registrardetallesplan_n.php?provi=<?php echo $provisional[0]?>&plan=<?php echo $cp ?>&nit=<?php echo $nit_empresa ?>&tp=<?php echo $tipo_proyecto ?>";
</script>