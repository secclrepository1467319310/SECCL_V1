    ...
    <body>
        <div id="">
            ...
            while ($row = oci_fetch_array($statement, OCI_BOTH)) {
                ...
                <table >
                    ...
                    <thead><tr><th><strong>Código Centro:</strong></th><td><?php echo $cen[0]; ?></td><th><strong>Nombre Centro:</strong></th><td><?php echo utf8_encode($nomcen[0]) ?></td><th><strong>Nombre Subdirector:</strong></th><td><?php echo utf8_encode($subdirector[0]) ?></td><th><strong>Nombre Responsable PECCL:</strong></th><td><?php echo utf8_encode($nom) . " " . utf8_encode($ape) ?></td></tr></thead>
        ...
        <div id="" >
            ...
            <?php
                ...
                $selectIndicadores = "SELECT * FROM INDICADORES WHERE CODIGO_CENTRO = $_GET[centro]";
                $parse = oci_parse($connection, $selectIndicadores);
                $resp = oci_execute($parse);
                $numRows = oci_fetch_all($parse, $rows);
            
            ?>
            <center>
                <form name="frmconcepto" accept-charset="UTF-8" method="post" action="guardar_concepto_general.php?plan=<?php echo $codiplan ?>">
            <table style="width: 400px">
                <tr><th colspan="10" >Metas 2016</th></tr>
                <tr>
                    <th width="5%">N° de Evaluaciones Meta 2016</th>
                    <th width="5%">N° de Personas Evaluadas Meta 2016</th>
<!--                    <th width="5%">Validadas 2015</th>
                ...
                <tr>
                    <td><input type="text" size="3" readonly onkeypress="return numeros(event)" name="num_certif_meta" value="<?php echo  $rows[META_CERTIFICADOS][0] ?>"></input></td>
                    <td><input type="text" size="3" readonly onkeypress="return numeros(event)" name="num_personas_certificadas" value="<?php echo $rows[META_PERSONAS][0] ?>"></input></td>
<!--                    <td><?php // echo $totalvalidada ?></td>
                    <td><input type="text" size="7" readonly onkeypress="return numeros(event)" name="pres_contratacion" value="<?php echo $pres_contratacion[0] ?>"></input></td>
                    <td><input type="text" size="7" readonly onkeypress="return numeros(event)" name="pres_gastos" value="<?php echo $pres_gastos[0] ?>"></input></td>
                    <td><?php // echo "$ ".$totalpres ?></td>
                    <td><input type="text" size="7" readonly onkeypress="return numeros(event)" name="pres_fal_contratacion" value="<?php echo $pres_fal_contratacion[0] ?>"></input></td>
                    <td><input type="text" size="7" readonly onkeypress="return numeros(event)" name="pres_fal_gastos" value="<?php echo $pres_fal_gastos[0] ?>"></input></td>
                    <td><?php // echo "$ ".$totalfal ?></td>
                    <td><textarea cols="30" name="concepto" readonly  rows="3"><?php echo $concepto[0] ?></textarea></td>-->
                </tr>
            </table>
                <br>
                </form>
                ...
                <table id='demotable1'>
                    ...
                    <thead>
                        <tr>
                            ...
                            <th>Consolidado TOTAL de Evaluar</th>
                            ...
                            <th>Observaciones</th>
                            <th>Generar Proyecto</th>
                            <th>Validado por Parte del Asesor</th>
                            <th>Concepto de la PAECCL (Asesor)</th>
                            <th>Editar</th>
                            <th>Detalle Proyecto Productivo</th>
                        </tr>
                    </thead>
                    <tbody>
                    </tbody>
                    ...
                    $query2 = "SELECT * FROM DETALLES_POA where ID_PLAN='$codiplan' and validacion <>0";
                    ...
                    while ($row2 = oci_fetch_array($statement2, OCI_BOTH)) {
                        ...
                        $query6 = ("SELECT id_proyecto FROM proyecto where id_provisional = '$row2[ID_PROVISIONAL]'");
...
//                        }
                        ...
                        echo "<td><textarea cols=\"30\" readonly rows=\"3\">$row2[OBSERVACIONES]</textarea></td></td>";
                            ...
                            if ($pro[0]!= null ) {
                                echo "<td><a id=\"deta\" name=\"deta\"  >Proyecto ya generado: $pro[0]</a></td>";
                            } else if ($pro[0]==null  && $row2[VALIDACION]==1){
                                echo "<td><a id=\"deta\" name=\"deta\" href=\"./registrar_proyecto_c.php?prov=" . $row2[ID_PROVISIONAL] . "\" >Generar Proyecto</a></td>";
                            } else {
                                echo "<td><a id=\"deta\" name=\"deta\"  >No Disponible</a></td>";
                            }
...
//                            echo "<td>Pend<input type=\"checkbox\" disabled value=\"Pendiente\" checked ></input><br>Si<input type=\"checkbox\" disabled value=\"Si\"><br>No</input><input type=\"checkbox\" disabled value=\"No\" ></input> </td>";
                        ...
                        echo "<td><textarea cols=\"30\" readonly rows=\"3\">$row2[CONCEPTO_ASESOR]</textarea></td></td>";
                            ...
                            echo "<td><a href=\"update_poa.php?plan=$codiplan&iddeta=$row2[ID_DETA]\"><img src=\"../images/editar.png\" alt=\"40\" width=\"40\"></a></td>";

                        echo "<td>$row2[DET_PRO_PRODUCTIVO]</td>";
