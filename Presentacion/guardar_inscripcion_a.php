...
$check = $_POST["check_apoyo"];
$obs_lider = $_POST["obs_apoyo"];

//---
$strSQL1 = "select CHEK_APOYO from inscripcion where id_proyecto=$proyecto and id_norma=$norma and id_candidato=$idca and grupo=$grupo";
$statement1 = oci_parse($connection, $strSQL1);
$resp1 = oci_execute($statement1);
$totalc = oci_fetch_array($statement1, OCI_BOTH);

if ($check == NULL && $totalc['CHEK_APOYO'] == 0) {
    $check = 0;
    $estado = 0;
} else if ($check == NULL && $totalc['CHEK_APOYO'] == 1) {
    $check = 1;
    $estado = 1;
}else if ($check == 1){
    $estado = 1;
...
//Insert historico
$strSQL2 = "update inscripcion set chek_apoyo='$check',estado='$estado'"
...
$strSQL14 = "select count(obs_apoyo) from observaciones_inscripcion where id_proyecto=$proyecto and id_norma=$norma and id_candidato=$idca and grupo=$grupo";
...
$strSQL22 = "INSERT INTO OBSERVACIONES_INSCRIPCION"
        . " (ID_PROYECTO,ID_CANDIDATO,ID_NORMA,GRUPO,OBS_APOYO) "
...
<script type="text/javascript">
    window.location = "../Presentacion/verificar_inscripcion_a.php?proyecto=<?php echo $proyecto ?>&idca=<?php echo $idca ?>&norma=<?php echo $norma ?>&grupo=<?php echo $grupo?>";
</script>
